...
const FunnelOverview: React.FC<FunnelOverviewProps> = ({ funnelData, onStageClick }) => {
    const stageColors = ['bg-blue-600', 'bg-sky-500', 'bg-teal-500'];
  ...
  return (
    <div className="bg-gray-800 rounded-xl shadow-lg h-full flex flex-col">
        ...
        <div className="flex flex-1">
            ...
            <div className="w-1/2 p-6 flex flex-col items-center justify-center space-y-2">
                {funnelStages.map((stage, index) => {
                    ...
                    >
                        <div 
                            className={`h-10 rounded-md flex items-center justify-center text-white font-bold text-sm transition-opacity group-hover:opacity-80 ${stageColors[index % stageColors.length]}`}
                            style={{ width: `${Math.max(widthPercentage, 25)}%` }}
                        >
                            {stage.value}
                        </div>
                        <p className="text-xs text-gray-400 mt-1 transition-colors group-hover:text-blue-400">{stage.name}</p>
                    </div>
                )})}
            </div>
